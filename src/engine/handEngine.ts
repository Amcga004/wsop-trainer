...
export interface SidePot {
  amount:         number
  eligibleSeats:  number[]  // seatIndexes that can win this pot
}
...
export interface HandEngine {
  ...
  // Side pot tracking
  sidePots:       SidePot[]
  deadMoney:      number        // chips returned to hero from uncalled excess
...
export function calcDeadMoney(seats: Seat[], heroSeatIndex: number): number {
  const hero = seats.find(s => s.seatIndex === heroSeatIndex)
  if (!hero) return 0
  const active = seats.filter(s => !s.folded && s.seatIndex !== heroSeatIndex)
  if (active.length === 0) return 0
  const maxOther = Math.max(...active.map(s => s.invested))
  return Math.max(0, hero.invested - maxOther)
}
...
export function buildSidePots(
  ...
  const hero = active.find(s => s.seatIndex === heroSeatIndex)
  const others = active.filter(s => s.seatIndex !== heroSeatIndex)
  if (!hero || others.length === 0) return []

  const maxOther = Math.max(...others.map(s => s.invested))
  if (maxOther >= hero.invested) {
    return [{ amount: totalPot, eligibleSeats: active.map(s => s.seatIndex) }]
  }

  // Hero invested more than any opponent — hero has an uncalled side pot
  const sidePotAmt = hero.invested - maxOther
  const mainPotAmt = totalPot - sidePotAmt
  return [
    { amount: mainPotAmt, eligibleSeats: active.map(s => s.seatIndex) },
    { amount: sidePotAmt, eligibleSeats: [heroSeatIndex] },
  ]
}

export function resolveShowdown(engine: HandEngine): void {
  ...
  if (engine.heroWon && !engine.isTie) {
    heroSeat.stack += totalPot
  } else if (engine.isTie) {
    const heroShare = Math.floor(totalPot / 2)
    heroSeat.stack += heroShare
    const tieVillain = engine.showdownSeat
    if (tieVillain) {
      const vs = engine.seats.find(s => s.seatIndex === tieVillain.seatIndex)
      if (vs) vs.stack += totalPot - heroShare
    }
  } else {
    const winner = engine.showdownSeat
      ?? stillIn.find(s => s.seatIndex !== heroSeat.seatIndex && !s.folded)
    if (winner) {
      const ws = engine.seats.find(s => s.seatIndex === winner.seatIndex)
...
export function processHeroAction(
  ...
  // Return uncalled excess to hero if any villain is all-in for less
  const activeVillains = stillIn.filter(s => s.seatIndex !== heroSeat.seatIndex)
  if (activeVillains.some(s => s.allIn) && heroSeat.invested > 0) {
    const maxVillainInvested = Math.max(...activeVillains.map(s => s.invested))
    const overbet = heroSeat.invested - maxVillainInvested
    if (overbet > 0) {
      heroSeat.stack += overbet
      heroSeat.invested -= overbet
      engine.pot -= overbet
      engine.deadMoney = overbet
    }
  }
